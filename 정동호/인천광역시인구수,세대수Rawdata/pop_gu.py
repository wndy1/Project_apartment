...
def popgu(gu_name):
    ...
    gu1=gu1.drop(gu1.columns[1:10],axis=1)
        ...
        for i in range(1,13): # 16년 ~ 18년 1월부터 12월까지
            try:
                if i < 10: # 1~9월까지
                    ...
                    gu=gu.drop(gu.columns[1:10],axis=1) # 필요없는 열 삭제
                    ...
                    gu=gu.drop(gu.columns[1:10],axis=1)
    ...
    return gu_res.to_csv("./인천광역시 "+str(gu_name)+" 인구수.csv",encoding='ms949')
...
def popgu2(gu_name):
    ...
    gu_res = gu_res.drop(gu_res.columns[0],axis=1)
    # gu_res = gu_res.drop(gu_res.index["행정기관"],axis=0)
    return gu_res.to_csv("./인천광역시 미추홀구 인구수.csv",encoding='ms949')
